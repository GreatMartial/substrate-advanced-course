...
fn create_kitty_work() {
	new_test_ext().execute_with(|| {
		...
		// 检查事件
		System::assert_last_event(mock::Event::KittiesModule(crate::Event::KittyCreate(1, 0)));
...
fn transfer_kitty_works() {
	new_test_ext().execute_with(|| {
		...
		// 检查event
		System::assert_last_event(mock::Event::KittiesModule(crate::Event::KittyTransfer(1, 2 , 0)));
		
		// 检查质押数量
		assert_eq!(Balances::reserved_balance(1), 0);
		assert_eq!(Balances::reserved_balance(2), 5_000);
	});
}
...
fn transfer_kitty_failed_when_not_self() {
	new_test_ext().execute_with(|| {
		run_to_block(10);
		let _ = KittiesModule::create(Origin::signed(1));
		assert_noop!(KittiesModule::transfer(Origin::signed(1), 1, 0), Error::<Test>::AlreadyOwned);
	})
}
...
fn breed_kitty_work() {
	new_test_ext().execute_with(|| {
		...
		// 检查质押数量
		assert_eq!(Balances::reserved_balance(1), 2 * 5_000);
		...
		// 检查质押数量
		assert_eq!(Balances::reserved_balance(1), 3 * 5_000);
	});
}
...
fn sale_kitty_work() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(mock::Event::KittiesModule(crate::Event::KittyForSale(1, 0, Some(5_000))));
	})
}
...
fn buy_kitty_work() {
	new_test_ext().execute_with(|| {
		...
		System::assert_last_event(mock::Event::KittiesModule(crate::Event::KittySaleOut(2, 0, Some(8_000))));

		// 检查释放已收到转账
		assert_eq!(Balances::free_balance(1), 100_000_000 + 8_000);
		// 检查是否已经转出
		assert_eq!(Balances::free_balance(2), 200_000_000 - 8_000 - 5_000);
